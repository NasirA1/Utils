...
namespace leak_detector
{
  ...
  inline char *
  truncate_left(char *dest, const size_t dest_size, const char *const src, const size_t src_size, const size_t src_count)
  {
  ...
  void insert(void *const ptr, const size_t size, const int tid, const char *const file, const size_t line)
  {
  ...
  void dump_findings()
  {
    ...
    for (const auto &entry: allocations)
      if (entry.ptr) {
        if (entry.line == 0)
          printf("%X\t%20p\t leaked   \t%13d\t\t%s\n", entry.tid, entry.ptr,
                 static_cast<int>(entry.size), entry.file);
        else
          printf("%X\t%20p\t leaked   \t%13d\t\t%s:%lu\n", entry.tid, entry.ptr,
                 static_cast<int>(entry.size), entry.file, entry.line);
        count++;
      }
      ...
      if (slot != -1) {
        if (line == 0)
          ...
          printf("%X\t%20p\t freed    \t%13d\t\t%s:%d\n", gettid(),
                 allocations[slot].ptr, static_cast<int>(-allocations[slot].size), short_filename,
  ...
  if (heap_profiling_started) {
    ...
    if (line == 0) {
      ...
      printf("%X\t%20p\t allocated\t %12d\t\t%s:%d\n",
             ...
             short_filename,
             static_cast<int>(line));
      insert(ptr, size, tid, short_filename, line);
    }
  }
