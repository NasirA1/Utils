...
public:
  ...
  virtual ~ThreadPool() throw() override = default;
...
public:
  ...
  ~BufferStream() throw() override = default;
...
public:
  virtual ~nocompressor() throw() = default;
...
inline void parse_query_text(const std::string &s, Params &params) {
  ...
  split(s.data(), s.data() + s.size(), '&', [&](const char *b, const char *e) {
    ...
    if (!key.empty()) {
      params.insert(std::pair<std::string, std::string>(decode_url(key, true), decode_url(val, true)));
    }
  });
}
...
inline SocketStream::~SocketStream() throw() {}
...
inline bool Server::read_content(Stream &strm, Request &req, Response &res) {
  ...
  if (read_content_core(
          ...
          [&](const MultipartFormData &file) {
            cur = req.files.insert(std::pair<std::string, MultipartFormData>(file.name, file));
            return true;
          },
          [&](const char *buf, size_t n) {
...
inline void Server::apply_ranges(const Request &req, Response &res,
  ...
  if (req.ranges.size() > 1) {
    ...
    res.headers.insert(std::pair<std::string, std::string>("Content-Type",
...
inline bool ClientImpl::write_request(Stream &strm, Request &req,
  ...
  if (close_connection) {
    if (!req.has_header("Connection")) {
      req.headers.insert(std::pair<std::string, std::string>("Connection", "close"));
    }
  }
  ...
  if (!req.has_header("Host")) {
    if (is_ssl()) {
      if (port_ == 443) {
        req.headers.insert(std::pair<std::string, std::string>("Host", host_));
      } else {
        req.headers.insert(std::pair<std::string, std::string>("Host", host_and_port_));
      }
    } else {
      if (port_ == 80) {
        req.headers.insert(std::pair<std::string, std::string>("Host", host_));
  ...
  if (!req.has_header("Accept")) { req.headers.insert(std::pair<std::string, std::string>("Accept", "*/*")); }

  if (!req.has_header("User-Agent")) {
    req.headers.insert(std::pair<std::string, std::string>("User-Agent", "cpp-httplib/0.9"));
  }
    ...
    if (req.content_provider_) {
      if (!req.is_chunked_content_provider_) {
        if (!req.has_header("Content-Length")) {
          ...
          req.headers.insert(std::pair<std::string, std::string>("Content-Length", length));
        }
      }
    } else {
      if (req.method == "POST" || req.method == "PUT" ||
          req.method == "PATCH") {
        req.headers.insert(std::pair<std::string, std::string>("Content-Length", "0"));
      }
    }
  } else {
    if (!req.has_header("Content-Type")) {
      req.headers.insert(std::pair<std::string, std::string>("Content-Type", "text/plain"));
    }
...
inline std::unique_ptr<Response> ClientImpl::send_with_content_provider(
  ...
  if (content_type) { req.headers.insert(std::pair<std::string, std::string>("Content-Type", content_type)); }

#ifdef CPPHTTPLIB_ZLIB_SUPPORT
  if (compress_) { req.headers.insert(std::pair<std::string, std::string>("Content-Encoding", "gzip")); }
#endif
...
#endif
  {
    if (content_provider) {
      ...
      req.headers.insert(std::pair<std::string, std::string>("Transfer-Encoding", "chunked"));
    } else {
